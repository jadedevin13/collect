...
class CsvDataExportPage extends Component {
  constructor(props) {
    ...
    this.handleCreateReportButtonClick = this.handleCreateReportButtonClick.bind(this)
    this.handleReportViewing = this.handleReportViewing.bind(this)
    ...
    ServiceFactory.recordService.startCSVDataExport(surveyId, parameters).then((job) => {
      this.props.dispatch(
        JobActions.startJobMonitor({
          jobId: job.id,
          title: "Report",
          okButtonLabel: "View Results",
          handleOkButtonClick: this.handleReportViewing,
        })
      )
    })
  }
  ...
  handleReportViewing(job) {
    if (job.completed) {
      ServiceFactory.recordService.getRecords().then(data => {
        this.setState({ ...this.state, records: data[0] })
      });
    }
      ...
      <Container>
        <Form>
          <FormGroup tag="fieldset">
            ...
            <FormGroup row  className="display-none-x">
          ...
          <Row>
            <Col sm={{ size: 'auto', offset: 5 }}>
              <Button onClick={this.handleCreateReportButtonClick} className="btn btn-success">
                Create Report
              </Button>
            </Col>
          </Row>
        ...
        {(records && records.length !== 0) && 
          <table className="styled-table">
            <thead>
              <tr>
                <th>Point of Generation</th>
                <th>Date</th>
                <th>Waste Type</th>
                <th>WM Staging Area</th>
                <th>Amount Managed</th>
                <th>Cumulative Amount Managed</th>
                <th>Units</th>
                <th>Waste Management Facility</th>
                <th>Comments</th>
              </tr>
            </thead>
            <tbody>
              {records.map(record => {
                return <tr key={record.point_of_generation_srs}>
                  <td>{record.point_of_generation_x},{record.point_of_generation_y}</td>
                  <td>{record.date_year}/{record.date_month}/{record.date_day}</td>
                  <td>{record.waste_type}/{record.sub_waste_type}</td>
                  <td>{record.wm_staging_area}</td>
                  <td></td>
                  <td></td>
                  <td>{record.units}</td>
                  <td>{record.waste_management_facility}</td>
